...
void ST7789_Select(void) {
    // set the CSX pin as a GPIO pin 
    GPIOPinWrite(ST7789_PIN_PORT_BASE, ST7789_CS_PIN, 0);
}
...
void ST7789_Deselect(void) {
    // complete
    GPIOPinWrite(ST7789_PIN_PORT_BASE, ST7789_CS_PIN, ST7789_CS_PIN);
}
...
void ST7789_SetData(void) {
    //complete
    GPIOPinWrite(ST7789_PIN_PORT_BASE, ST7789_DC_PIN, ST7789_DC_PIN);
}
...
void ST7789_SetCommand(void) {
    GPIOPinWrite(ST7789_PIN_PORT_BASE, ST7789_DC_PIN, 0);
}
...
void ST7789_WriteCommand(uint8_t cmd) {
    // set command mode, write via SPI, then set the data on the ST7789
    ST7789_SetCommand();
...
void ST7789_WriteData(uint8_t data) {
    // data mode already set before, so write the data using SPI 
    ST7789_SetData();
...
void ST7789_Init() {
    ...
    // note: random software delays (SysCtlDelay) are beneficial
    // note 2: sometimes 
    // select
    ST7789_Select();

    // write reset reg (software reset)
    ...
    // edit caset (column origin)
    ST7789_WriteCommand(ST7789_CASET_ADDR);
    ST7789_WriteData(0x00);
    ST7789_WriteData(0x00);
    ST7789_WriteData((X_MAX-1)>>8);
    ST7789_WriteData((X_MAX-1));
    delay_ms(10);


    // edit raset (row origin)
    ST7789_WriteCommand(ST7789_RASET_ADDR);
    ST7789_WriteData(0x00);
    ST7789_WriteData(20);
    ST7789_WriteData(((Y_MAX - 1)>>8));
    ST7789_WriteData((Y_MAX - 1));
    delay_ms(10);
